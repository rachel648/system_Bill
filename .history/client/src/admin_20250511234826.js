...
const Admin = () => {
  ...
  const [payments, setPayments] = useState({ today: 0, month: 0, year: 0 });
  ...
  const fetchPaymentsData = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/payments/stats'); // Replace with actual API
  ...
  const fetchClientPayments = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/payments'); // Replace with actual API
      setPayments(res.data.paymentsSummary || { today: 0, month: 0, year: 0 });
  ...
  const handleAction = async (clientId, action) => {
    setLoading(true);
    try {
      await axios.post(`/api/clients/${action}`, { clientId });
      setStatus({
        message: `${action.charAt(0).toUpperCase() + action.slice(1)} successful!`,
        type: 'success',
      });
      fetchClientPayments(); // Refresh
    } catch (err) {
      setStatus({
  ...
  const renderClientList = () => {
    return clients.map((client) => (
      <tr key={client.id}>
        ...
        <td style={styles.tableCell}>{client.paymentDate}</td>
        <td style={styles.tableCell}>{client.amount}</td>
      ...
      <div style={styles.paymentStats}>
        ...
        <div style={styles.statsContainer}>
          <div style={styles.statBox}>
            ...
            <p>{payments?.today || 0} Payments</p>
          </div>
          <div style={styles.statBox}>
            <h4>This Month's Payments</h4>
            <p>{payments?.month || 0} Payments</p>
          </div>
          <div style={styles.statBox}>
            <h4>This Year's Payments</h4>
            <p>{payments?.year || 0} Payments</p>
          </div>
        </div>
      </div>
      ...
      <table style={styles.table}>
        <thead style={styles.tableHeader}>
          <tr>
            ...
            <th style={styles.tableCell}>Payment Date</th>
