...
const Packages = () => {
  ...
  return (
    <div className="container">
      <h1 className="main-title">ELITE NETWORKS HOTSPOT</h1>
      ...
      <div className="card-container">
        {Object.keys(packagePrices).map((pkg) => (
          <div key={pkg} className={`card ${pkg.toLowerCase().replace(' ', '-')}-card`} onClick={() => handleCardClick(pkg)}>
            <h3>{pkg}</h3>
            <p>KSh {packagePrices[pkg].toLocaleString()}</p>
            <p>{pkg === '1 Hour' ? '1' : 
                pkg === '3 Hours' ? '3' : 
                pkg === '4 Hours' ? '4' : 
                pkg === '5 Hours' ? '5' : 
                pkg === '24 Hours' ? '24' : 'Special'} Hour{pkg === '1 Hour' ? '' : 's'} WiFi Access</p>
          </div>
        ))}
      </div>
