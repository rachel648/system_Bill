...
app.post('/api/payment/test-complete', async (req, res) => {
  try {
    ...
    console.log(`✅ Payment auto-completed for testing: ${payment.phoneNumber}`);
    
    res.json({
      success: true,
      message: 'Payment auto-completed for testing',
      payment
    });
...
app.post('/callback', async (req, res) => {
  ...
  try {
    ...
    if (!payment) {
      ...
      // Try to find latest pending payment
      const pendingPayments = await Payment.find({ status: 'pending' }).sort({ createdAt: -1 }).limit(5);
      console.log(`🔍 Found ${pendingPayments.length} pending payments`);
      
      if (pendingPayments.length > 0) {
        // Use the most recent pending payment
        payment = pendingPayments[0];
        console.log(`🔗 Linking to payment: ${payment._id}, Phone: ${payment.phoneNumber}`);
    ...
    if (resultCode === 0) {
      // Payment successful
      console.log(`✅ Payment successful for: ${payment.phoneNumber}`);
      ...
      if (phoneItem && phoneItem.Value) {
        ...
        console.log(`📱 Phone updated: ${payment.phoneNumber}`);
      }
      ...
      console.log(`💾 Payment saved: ${payment.phoneNumber}, Package: ${payment.package}`);
      console.log(`⏰ Start: ${payment.startTime}, End: ${payment.endTime}`);
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    totalPayments: 0 // Will be updated in async way
  });
});
...
connectDB().then(() => {
  const server = app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('🚀 ELITE NETWORKS HOTSPOT BACKEND');
    ...
    if (process.env.MPESA_CALLBACK_URL) {
      ...
      console.log('⚠️  MPESA_CALLBACK_URL not set in .env');
    }
    ...
    console.log(`   GET  /api/admin/payments   - Admin payments (requires auth)`);
  ...
  server.on('error', (err) => {
    ...
    if (err.code === 'EADDRINUSE') {
      ...
      console.error('Try: PORT=5001 in .env file');
    }
  });
