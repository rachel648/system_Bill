  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (existingUser) {
        ...
        setTimeout(() => {
          navigate(`/${existingUser.serviceType}`); // Redirect to WiFi or Hotspot based on their service type
        }, 1000);
        return;
      }
    } catch (err) {
    ...
    try {
      ...
      if (formData.serviceType === 'wifi') {
        ...
        // Navigate to a placeholder or Hotspot service page (for now, you can create a simple placeholder page)
        navigate('/hotspot'); // Change this route as you need
        setStatus({
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (user && user.serviceType) {
        ...
        setTimeout(() => {
          navigate(`/${user.serviceType}`); // Navigate to the user's chosen service (wifi or hotspot)
        }, 1000);
      } else {
