...
const ServiceForm = () => {
  const [formData, setFormData] = useState({
    username: '',
    serviceType: 'wifi',
    servicePackage: 'basic',  // Renamed from 'packageType' to 'servicePackage'
    duration: 1,
    amount: 10
  });
  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await axios.post('/api/users/register', {
        username: normalizedUsername,  // Use lowercase username
        serviceType: formData.serviceType,
        package: formData.servicePackage,  // Updated property name here
  ...
  const handleSignIn = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`/api/users/status/${normalizedUsername}`);  // Use lowercase username
      const user = res.data;
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.inputGroup}>
        ...
        <select
          name="servicePackage"  // Updated variable name here
          value={formData.servicePackage}  // Updated variable name here
          onChange={handleChange}
          style={styles.select}
        >
